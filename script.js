const students = [
    {
      "id": "1",
      "name": "Asmina M",
      "email": "Asminamohamed21@yahoo.com"
    },
    {
      "id": "2",
      "name": "Wiliam Weber",
  ...
  //copy of data 
   let back_up = students;
  // console.log(back-up);

  let noOfPages = students.length/10;
  let currentPage = 1;
...
paginate(currentPage)
  ...
  function paginate(pageNumber){
    ...
    let paginatedData = students.slice(startIndex,endIndex);
    
    document.getElementById("pageInfo").innerText = currentPage+" of "+noOfPages;
  ...
  for (let i = 0;i<paginatedData.length;i++){
   ...
   // first create a row tr
  // create id in td(table data)
  // put the td in tr(table row)
  // put tr in tbody(table body)


        let row = document.createElement("tr");

        let id = document.createElement("td");
        id.innerText = startIndex+i+1;
      
        let names = document.createElement("td");
        names.innerText = paginatedData[i].name;
      
        let email = document.createElement("td");
        email.innerText = paginatedData[i].email;
      
        row.append(id,names,email);
